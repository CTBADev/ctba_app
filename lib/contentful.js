...
export async function getGames() {
  const response = await client.getEntries({
    content_type: "game",
    include: 2, // This will include linked entries (teams, divisions, venues)
    order: "fields.fixtureDate",
  });

  return response.items.map((item) => ({
    sys: item.sys,
    gameNumber: item.fields.gameNumber?.["en-US"],
    teamA: item.fields.teamA?.["en-US"],
    teamB: item.fields.teamB?.["en-US"],
    ageGroup: item.fields.ageGroup?.["en-US"],
    venue: item.fields.venue?.["en-US"],
    courtNumber: item.fields.courtNumber?.["en-US"],
    fixtureDate: item.fields.fixtureDate?.["en-US"],
    scoreA: item.fields.scoreA?.["en-US"],
    scoreB: item.fields.scoreB?.["en-US"],
    isLocked: item.fields.isLocked?.["en-US"],
  }));
}
...
export async function getVenues() {
  ...
  return response.items.map((item) => ({
    ...
    physicalAddress: item.fields.physicalAddress?.["en-US"],
    googleMapLink: item.fields.googleMapLink?.["en-US"],
  }));
}
